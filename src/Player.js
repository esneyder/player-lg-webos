...
const DRM_SYSTEM_ID = 'urn:dvb:casystemid:19219';
 ...
 const onStreamNotFound = 'onStreamNotFound';
 const onConnectionTimeout = 'onConnectionTimeout';
 const onConnectionFailed = 'onConnectionFailed';
 const onNetworkDisconnected = 'onNetworkDisconnected';
 const onAuthenticationFailed = 'onAuthenticationFailed';
 const onUnknownError = 'onUnknownError';
 const onRenderError = 'onRenderError';
...
class Web0sPlayer {
    ...
    createPlayer(options) {
        ...
        if (!this.streamIsImage()) {
            ...
            this._playerHTMLTag = window.document.createElement(
                this.streamIsAudio() ? 'audio' : 'video'
            );
        ...
        const vWrapper = document.getElementById(this.options.parentWrapper.id);
            ...
            if (!this._playerHTMLTag) {
                ...
                if (!this.streamIsImage()) {
                    this.setPlayerBackground(
                        options.backgroundImage ? options.backgroundImage : null
                    );
                }
                if (options.muted) {
                    this._playerHTMLTag.setAttribute('muted', 'muted');
    ...
    async getDrmUrl(manifest) {
        ...
        if (manifest) {
            ...
            const drmurlResult = await RequestManager.addRequest(drmTask);
    ...
    async loadMedia() {
        ...
        if (this.options.src) {
            ...
            if (this.options.streamType === 'smooth_streaming') {
                ...
                if (
                    !this.options.drmInfo.server_url &&
                    this.options.provider.toUpperCase() !== 'HBO'
                ) {
                    this.options.drmInfo.server_url = await this.getDrmUrl(this.options.src);
                }

                const { server_url } = this.options.drmInfo;
                const challenge = (this.options.drmInfo && this.options.drmInfo.challenge) || '';
                const content_id = (this.options.drmInfo && this.options.drmInfo.content_id) || '';
                const device_id = this.options.drmInfo.device_id || '';
                ...
                if (this.options.drmInfo.server_url) {
                    this.web0sInitiatorMsg = this.getXMLInitiator(
                        this.options.drmInfo.server_url,
                        customData
                    );
    ...
    setVideoSource(url, type) {
        ...
        if (this.options.streamType === 'smooth_streaming' || this.options.streamType === 'smooth_streaming_ma') {
            const options = {};
            options.option = {};
            let mediaOption = '';
    ...
    async replaceMediaSource(newSource) {
        ...
        const currentSource = document.getElementById(
            `Html5PlayerSource${this.options.isPip ? '_pip' : '_full'}`
        );
        if (currentSource) {
        ...
        if (this.options.streamType === 'hls') {
    ...
    seekTimeshift(timeshiftData, firstSeek) {
        ...
        if (firstSeek) {
            ...
            console.log(
                '[WEB0S PLAYER] trying to do timeshift seek to: <secondTo/seekWindow>  ',
                secondTo,
                this.seekWindow
            );
            this.seek(secondTo);
    ...
    async tryOnFirstTimeshiftSeek(currentTry, timeshiftData) {
        console.log(
            `[WEB0S PLAYER]> tryOnFirstTimeshiftSeek getCurrentTime: [${this.getCurrentTime()}]`
        );
    ...
    */
    ...
    destroyPip() {
        ...
        if (!this.streamIsImage()) {
            const source_dom = document.getElementById('Html5PlayerSource_pip');
            if (source_dom) {
    ...
    unloadWebosDrmClient() {
        ...
        if (this.web0sClientId) {
            ...
            const requestt = this.webOS.service.request('luna://com.webos.service.drm', {
                method: 'unload',
        ...
        this.setInitiatorInformation()
            .then(resp => {
                ...
                this.sendDRMInformation()
                    ...
                    .catch(msg => {
                        console.log(
                            '[WEB0S PLAYER] error en las promesas en sendDRMInformation',
                            msg
                        );
    ...
    setInitiatorInformation() {
        return new Promise((resolve, reject) => {
            this.loadDrmClient()
                .then(clientId => {
                    console.log(
                        '[WEB0S PLAYER] resueltas todas las promesas en setInitiatorInformation'
                    );
                    const resp_alt = [];
    ...
    loadDrmClient() {
        ...
        return new Promise((resolve, reject) => {
            const requestt = this.webOS.service.request('luna://com.webos.service.drm', {
                ...
                onFailure: result => {
                    console.log(
                        `[WEB0S PLAYER] loadDrmClient [${result.errorCode}] ${result.errorText}`
                    );
                    const err = `${result.errorCode}<>${result.errorText}`;
    ...
    disableHTML5AudioTracks() {
        ...
        if (this.options.streamType === 'smooth_streaming' && this.audioTracks && this._playerHTMLTag) {
            try {
                console.log(
                    '[WEB0S PLAYER] disableHTML5AudioTracks, es multiple, audioTracks> ',
                    this._playerHTMLTag.audioTracks
                );
                ...
                console.log(
                    '[WEB0S PLAYER] disableHTML5AudioTracks catch error (it does not matter, maybe this is LG webos <= 2014)> ',
                    e
                );
            }
        }
    ...
    async tryGetAudioTracks(currentTry) {
        ...
        console.log(
            `[WEB0S PLAYER]> tryToGetAudioTracks retry number: [${currentTry}] - player audioTracks is READY`
        );
    ...
    setAudioTrack(codeTrack) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const id = parseInt(codeTrack, 10);
            const internalAudioIndex = this.getAudioIndexOfCode(codeTrack);
            if (internalAudioIndex === null) {
                reject('Dont have audioTracks information, index lang does not exist');
            } else if (this.currentAudioTrackIndex === internalAudioIndex) {
                ...
                console.log(
                    '[WEB0S PLAYER] setAudioTrack saving currentTime> ',
                    this.seek_resume,
                    currentTime,
                ...
                console.info(
                    `[WEB0S PLAYER] Set audioTrack to index: ${internalAudioIndex}, lang code: ${codeTrack}`
                );
                const pid = this.getWebosMediaId();
                ...
                this.webOS.service.request('luna://com.webos.media', {
                    ...
                    onSuccess: () => {
                        console.log(
                            '[WEB0S PLAYER] success when selectAudioTrack, calling callbackonSuccessChangeAudio, params codeTrack & seekTime> ',
                            codeTrack,
                            currentTime
                        );
                    ...
                    onFailure: err => {
                        console.log(
                            '[WEB0S PLAYER] An error occured when changing audio track... :('
                        );
    ...
    onSuccessChangeAudio(codeTrack, seekTime) {
        console.log(
            '[WEB0S PLAYER] enter onSuccessChangeAudio, codeTrack & seekTime> ',
            codeTrack,
            seekTime
        );
